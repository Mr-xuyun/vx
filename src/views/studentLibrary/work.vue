<template>
  <div>
    ...
    <!--  滑选是否已评改-->
    <div>
      <mt-cell title="滑选是否已评改">
        <img
            slot="icon"
            src="../../assets/student/修改.png"
            width="20"
            height="20"
        />
        <van-switch
    ...
    <van-checkbox-group v-model="result" ref="checkboxGroup">
      <div class="difficultList" v-for="(item, i) of difficultList" :key="i">
        <div :style="{ borderColor: checkedAll == true ? '#1b8fff80' : '#f2f2f2' }">
          <div @click="$router.push('/studentDetails/'+item.id+'/homework_base')">
            <div>
...
export default {
  ...
  methods: {
    ...
    del() {
      if (this.result.length == 0) {
        ...
        this.$dialog.confirm({title: " 确认删除吗", confirmButtonColor: "#1B8FFF",}).then(() => {
          this.axios.post("vx/homeworkBase/deleteBatch", this.result).then((res) => {
            let result = res.data;
    ...
    vanSwitch() {
      this.checkedCorrect = !this.checkedCorrect;
      this.getDifficultHomework();
    },
    //搜索
    onSearch(val) {
      ...
      this.axios.post("vx/homeworkBase/list", data).then((res) => {
        let result = res.data;
    ...
    getDifficultHomework() {
      ...
      let data = {
        ...
        startDate: startDate,
        endDate: endDate,
        source: this.workSource,
        isAnalysed: this.checkedCorrect,
      };
      this.$toast.loading({
        title: "加载中...",
        duration: 0,
    ...
    reloadAll() {
      ...
      this.axios.post("vx/homeworkBase/list", {token: this.getLocalStorage("token"),}).then((res) => {
        if (res.data.result.records.length != 0) {
          this.difficultList = res.data.result.records;
          this.$toast.clear();
        } else {
  ...
  created() {
    ...
    this.axios.post("vx/coursePublish/findCategoryByName", {name: "作业来源"}).then((res => {
      for (let subject of res.data.result) {
        this.sourceList.push(subject.title)
      }
    }))
  },
  mounted() {
