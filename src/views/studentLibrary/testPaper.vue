    ...
    <van-checkbox-group v-model="result" ref="checkboxGroup">
        ...
        <div class="difficultList" v-for="(item, i) of difficultList" :key="i">
          ...
          >
            <div
              @click="
                $router.push('/studentDetails/' + item.id + '/test_papers')
              "
            >
              <div>
...
export default {
  ...
  data() {
    return {
      ...
      testTypeName: "", // 试卷类型
      testSourceName: "", // 试卷来源
      testTypeList: [{ text: "试卷类型", value: 0 }],
      testSourceList: [{ text: "试卷来源", value: "a" }],
      loading: false,
      finished: false,
      pageSize: 5,
      pageNo: 1,
  ...
  methods: {
    ...
    // 试卷来源
    testSourceFun() {
      if (this.testSource != "a") {
        for (let item of this.testSourceList) {
          if (this.testSource == item.value) {
            this.testSourceName = item.text;
          }
        }
      } else {
        this.testSourceName = "";
    ...
    testTypeFun() {
      if (this.testType != 0) {
        for (let item of this.testTypeList) {
          if (this.testType == item.value) {
            this.testTypeName = item.text;
          }
        }
      } else {
        this.testTypeName = "";
    ...
    del() {
      if (this.result.length == 0) {
        ...
        this.$dialog
          ...
          .then(() => {
            this.axios
              .post("vx/testPapers/deleteBatch", this.result)
              .then((res) => {
                let result = res.data;
  ...
  created() {
    ...
    this.axios
      ...
      .then((res) => {
        for (let test of res.data.result) {
          let obj = { text: test.title, value: test.key };
          this.testSourceList.push(obj);
        }
      });
    this.axios
